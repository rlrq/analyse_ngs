...
parser.add_argument('--screen-grna', action="store_true", dest="screen_grna", default=False,
                    help="screen for mutations across gRNA length (+1 bp 3' if gRNA length is odd)")
...
def run_crispresso(row):
    ...
    try:
        ...
        ## set window to cover full gRNA if args.screen_grna
        if screen_grna:
            import math
            crispresso_command.extend(['--cleavage_offset', str(-int(len(guide)/2)),
                                       '--quantification_window_size', str(math.ceil(len(guide)/2))])
